...
def getPathToCurrentDir(*nameOfProjectFolder):
    ...
    actualPath = "" #WHEN FUNCTION IS PARAMETERIZED
    ...
    actualPath = "" #FOR WHEN ARGS IS EMPTY
    if listWithPossiblePlaces:
        line = str(listWithPossiblePlaces[0]).split(splitBy)
        for y in range(len(line) - 1):
            actualPath = actualPath + line[y] + splitBy
        return actualPath

    return listWithPossiblePlaces[0]
